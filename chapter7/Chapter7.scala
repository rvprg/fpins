...
object Chapter7 extends App {
  ...
  object Par {
    ...
    // 7.1
    // def map2[A, B, C](a: Par[A], Par[B])(f: (A, B) => C): Par[C] = ...
    ...
    // 7.4
    def asyncF_0[A, B](f: A => B): A => Par[B] = a => fork(map(unit(a))(f))

    def asyncF[A, B](f: A => B): A => Par[B] = a => lazyUnit(f(a))

    // 7.5
    def sequence[A](ps: List[Par[A]]): Par[List[A]] =
      ps.foldRight[Par[List[A]]](unit(List()))((x, y) => map2(x, y)((x, y) => x :: y))

    // 7.6
    def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] = {
      as.map(asyncF(x => (f(x), x)))
        .foldRight[Par[List[A]]](unit(List()))((x, y) => map2(x, y)((x, y) => if (x._1) x._2 :: y else y))
    }
